use crate::whirlpool::utils::kamino::{
    KAMINO_ADDITIONAL_COMPUTE_UNITS, get_kamino_flashloan_borrow_ix, get_kamino_flashloan_repay_ix,
};
use crate::whirlpool::utils::pools::MintPoolData;
use crate::whirlpool::utils::raydium::{raydium_authority, raydium_cp_authority};
use crate::whirlpool::utils::utils::Config;
...
use crate::whirlpool::constants::sol_mint;
use crate::whirlpool::constants::whirlpool_program_id;
use crate::whirlpool::constants::{dlmm_event_authority, dlmm_program_id};
use crate::whirlpool::constants::{pump_fee_wallet, pump_program_id};
use crate::whirlpool::constants::{
...
pub fn whirlpool_buy(
    wallet: &Keypair,
    pd: &MintPoolData,
    min_profit: u64,
    max_bins: u64,
) -> anyhow::Result<Instruction> {
    if pd.whirlpool_pools.is_empty() {
        bail!("no Whirlpool pools supplied");
    }

    // ----- constants -----
    let exec: Pubkey = Pubkey::from_str("MEViEnscUm6tsQRoGd9h6nLQaQspKj7DB2M5FwM3Xvz")?;
    let fee = Pubkey::from_str("6AGB9kqgSp2mQXwYpdrV4QVV8urvCaDS35U1wsLssy6H")?;
    ...
    let mut accs = vec![
        ...
        AccountMeta::new(pd.wallet_wsol_account, false),
        AccountMeta::new_readonly(token_program_id, false),
        AccountMeta::new_readonly(system_program::ID, false),
        AccountMeta::new_readonly(associated_token_program_id, false),
    ];

    // X-mint block (Whirlpool only)
    accs.push(AccountMeta::new_readonly(pd.mint, false));
    let x_ata =
        spl_associated_token_account::get_associated_token_address(&wallet.pubkey(), &pd.mint);
    accs.push(AccountMeta::new(x_ata, false));

    for pool in &pd.whirlpool_pools {
        accs.push(AccountMeta::new_readonly(whirlpool_program_id(), false));
        accs.push(AccountMeta::new(pool.pool, false));
        accs.push(AccountMeta::new(pool.oracle, false));
        accs.push(AccountMeta::new(pool.x_vault, false));
        accs.push(AccountMeta::new(pool.y_vault, false));
        for ta in &pool.tick_arrays {
    ...
    data.extend_from_slice(&max_bins.to_le_bytes());
