...
mod rpc_utils {
    ...
    pub async fn slot_subscribe_loop(
        ...
        while !exit.load(Ordering::Relaxed) {
            ...
            match PubsubClient::new(
                "wss://mainnet.helius-rpc.com/?api-key=f44bbff8-2b1c-450d-bd79-e9ab5ec60ba9",
            )
            .await
            {
                    ...
                    Ok((mut slot_update_subscription, _unsubscribe_fn)) => {
                        while let Some(slot_update) = slot_update_subscription.next().await {
                            if let SlotUpdate::FirstShredReceived { slot, timestamp: _ } =
                                slot_update
                            {
                                info!("slot_subscribe_slot slot {}", slot);
                                if slot_sender.send(slot).await.is_err() {
                                    error!("slot_subscribe_send_error errors {}", 1,);
                        ...
                        error!(
                            "slot_subscribe_disconnect_error errors {}",
                            slot_subscribe_disconnect_errors,
                        );
                    }
                    Err(e) => {
                        slot_subscribe_errors += 1;
                        error!(
                            "slot_subscribe_error errors {} error str {}",
                            slot_subscribe_errors,
                            e.to_string(),
                        );
                    }
                },
                Err(e) => {
                    connect_errors += 1;
                    error!(
                        "slot_subscribe_pubsub_connect_error errors {} error str {}",
                        connect_errors,
                        e.to_string(),
                    );
                }
